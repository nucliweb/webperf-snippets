...
sequenceDiagram
    ...
    Note over Browser: ⏳ Input Delay<br/>(waiting for main thread)

    Browser->>Handler: Start processing
    Note over Handler: ⚙️ Processing Time<br/>(event handler execution)
    Handler->>Handler: Run JavaScript
    Handler->>Handler: Update state

    Handler->>Renderer: Request render
    Note over Renderer: 🎨 Presentation Delay<br/>(render + paint)
...
```mermaid
flowchart LR
    Start([👆 User<br/>Input]) --> Phase1["⏳ Input Delay<br/><small>Waiting for main thread</small>"]
    Phase1 --> Phase2["⚙️ Processing Time<br/><small>Event handler execution</small>"]
    Phase2 --> Phase3["🎨 Presentation Delay<br/><small>Render + Paint</small>"]
    Phase3 --> End([✅ Visual<br/>Feedback])
...
flowchart TD
    ...
    InputDelay -->|Yes| InputFix["⚡ Break up long tasks<br/>⚡ Use scheduler.yield()<br/>⚡ Defer non-critical work<br/>⚡ Use requestIdleCallback"]

    ProcessTime -->|Yes| ProcessFix["🔧 Optimize event handlers<br/>🔧 Debounce rapid events<br/>🔧 Move work to Web Workers<br/>🔧 Reduce JavaScript complexity"]

    PresDelay -->|Yes| PresFix["🎨 Reduce DOM updates<br/>🎨 Avoid layout thrashing<br/>🎨 Use content-visibility<br/>🎨 Batch DOM changes"]
