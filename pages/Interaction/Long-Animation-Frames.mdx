...
```mermaid
graph TB
    subgraph LT[" Long Tasks API ❌ "]
        LT1["⏱️ Task > 50ms"]
        LT2["❓ Limited Attribution"]
        LT3["🚫 No Render Time"]
    end

    subgraph LA[" Long Animation Frames API ✅ "]
        LA1["⏱️ Frame > 50ms"]
        LA2["✅ Full Script Attribution"]
        LA3["🎨 Includes Render Time"]
        LA4["🔍 Detects Forced Layouts"]
    end
