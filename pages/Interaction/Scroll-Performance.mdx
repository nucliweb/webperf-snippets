...
```mermaid
graph TB
    subgraph NP[" ❌ Non-passive (default) "]
        direction TB
        NP1["🖱️ User scrolls"] --> NP2["⏳ Wait for JS\n(main thread)"]
        NP2 --> NP3["🎨 Compositor moves page"]
        NP3 --> NP4["Frame rendered"]
    end

    subgraph P[" ✅ passive: true "]
        direction TB
        P1["🖱️ User scrolls"] --> P2["🎨 Compositor moves page\n(immediately)"]
        P1 --> P3["⚙️ JS runs in parallel\n(main thread)"]
