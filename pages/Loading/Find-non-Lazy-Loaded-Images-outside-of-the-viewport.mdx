...
- **Delay LCP and TTI** by competing for network and CPU resources
...
function findImgCandidatesForLazyLoading() {
  ...
  if (imagesWithoutSize > 0) {
    console.log(
      `   ⚠️ ${imagesWithoutSize} image(s) not found in Performance API (size unknown)`,
    );
  }
...
```
✅ Good job, the site has all the images out of the viewport with lazyloading.
```
