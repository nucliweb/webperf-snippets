...
2. **`analyzeContentVisibilityOpportunities()`** - Identifies elements that could benefit from `content-visibility: auto` based on their position and size
...
function analyzeContentVisibilityOpportunities() {
  const viewportHeight = window.innerHeight;
  const opportunities = [];

  // Elements that are good candidates for content-visibility: auto
  ...
  // Find large offscreen elements
  function analyzeLargeElements() {
    const allElements = document.querySelectorAll("section, article, div, aside, footer, main > *");
    ...
    allElements.forEach((el) => {
      ...
      if (cv && cv !== "visible") return;

      // Skip small elements
      if (rect.height < 200) return;

      // Check if element is below the fold
      const isOffscreen = rect.top > viewportHeight * 1.5;

      if (isOffscreen && rect.height > 0) {
        ...
        opportunities.push({
          ...
          estimatedSavings: estimateRenderSavings(el, childCount),
        });
      }
    });
  }
  ...
  function estimateRenderSavings(element, childCount) {
    // Rough estimation based on child count and element size
    // Each element costs approximately 0.1-0.5ms to render
  ...
  analyzeLargeElements();

  // Sort by estimated impact
  opportunities.sort((a, b) => {
    const aChildren = parseInt(a.childElements);
    const bChildren = parseInt(b.childElements);
  ...
  if (opportunities.length === 0) {
    console.log("%c✅ No major opportunities found.", "color: #22c55e; font-weight: bold");
    console.log("Most large elements are either in the viewport or already optimized.");
  } else {
    console.log(
      `Found ${opportunities.length} element(s) that could benefit from content-visibility: auto`,
    );
    console.log("");
    console.table(opportunities.slice(0, 15)); // Show top 15

    if (opportunities.length > 15) {
      console.log(`... and ${opportunities.length - 15} more elements`);
    }
    ...
    console.log(
      "%c/* Optimize offscreen content */\n" +
        ...
        "  contain-intrinsic-size: auto 500px; /* Adjust height estimate */\n" +
        "}",
...
// Uncomment to also find optimization opportunities:
...
#### `analyzeContentVisibilityOpportunities()` Results

This function identifies elements that would benefit from `content-visibility: auto`:

- **selector**: Element identifier
- **height**: Current height (important for `contain-intrinsic-size`)
- **distanceFromViewport**: How far below the fold the element is
