...
(() => {
  ...
  if (preconnectLinks.length === 0) {
    ...
    const preconnectResults = preconnectLinks.map(link => {
      ...
      return { domain, href, wasUsed, requestCount, link };
    });

    const usedPreconnects = preconnectResults.filter(r => r.wasUsed);
    const unusedPreconnects = preconnectResults.filter(r => !r.wasUsed);
    ...
    const usageRate = Math.round((usedPreconnects.length / preconnectLinks.length) * 100);
    console.log("");
    console.log("%cUsage Rate: " + usageRate + "%", "font-weight: bold;");
  ...
  if (dnsPrefetchLinks.length === 0) {
    ...
    const unusedDnsPrefetch = dnsPrefetchResults.filter(r => !r.wasUsed);
    ...
    const usageRate = Math.round((usedDnsPrefetch.length / dnsPrefetchLinks.length) * 100);
    console.log("");
    console.log("%cUsage Rate: " + usageRate + "%", "font-weight: bold;");
  }
  ...
  const totalIssues = preloadLinks.filter(l => !usedResourceUrls.has(normalizeUrl(l.href))).length +
    preconnectLinks.filter(l => {
      ...
      return domain && !usedDomains.has(domain);
    }).length +
