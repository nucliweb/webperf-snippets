...
(() => {
  ...
  if (preloadedScripts.length === 0) {
    console.log("%c✅ No preloaded scripts found.", "color: #22c55e; font-weight: bold;");
    return;
  }
  ...
  preloadedScripts.forEach((preload) => {
    ...
    if (!matchingScript) {
      ...
      const location = inHead ? "head" : "body";
  ...
  if (errors.length === 0 && warnings.length === 0) {
    console.log("");
    console.log(
      "%c✅ All preloaded scripts look properly configured!",
      "color: #22c55e; font-weight: bold;",
    );
  } else {
    console.log("");
...
This can delay your LCP by 100-500ms or more on slower connections.
