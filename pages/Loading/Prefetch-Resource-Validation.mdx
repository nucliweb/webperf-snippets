...
(() => {
  ...
  prefetchLinks.forEach((link) => {
    ...
    if (perfEntry) {
      ...
      analysis.cacheStatus = perfEntry.transferSize === 0 ? "cached" : "network";
  ...
  console.log(`   Loaded resources: ${issues.length + validPrefetch.length}`);
...
| **Status**   | Whether resource was loaded or is pending      |
| **Cache**    | Cache status (cached vs network fetch)         |
| **Issue**    | Specific problems detected                     |
